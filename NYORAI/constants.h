...
int 		  didInitDs = 0;
GsOT 		  orderingTable[5];
GsOT_TAG  	  minorOrderingTable[5][1<<OT_LENGTH];
...
void sprite_create(unsigned char imageData[], int x, int y, GsSPRITE **sprite) {
	...
	*sprite = malloc3(sizeof(GsSPRITE));
	...
	(*sprite)->scalex = (ONE * 1)/2;				// Sprite scale (width)
	(*sprite)->scaley = (ONE * 1)/2;				// Sprite scale (height)
...
void sprite_create_map(unsigned char imageData[], int x, int y, GsSPRITE **sprite) {
...
void sprite_create_menu(unsigned char imageData[], int x, int y, GsSPRITE **sprite) {
	...
	*sprite = malloc3(sizeof(GsSPRITE));
	...
	(*sprite)->mx = 0;                          // Rotation x coord
	(*sprite)->my = 0;                          // Rotation y coord
	(*sprite)->scalex = (ONE * 1)*1.35;				// Sprite scale (width)
	(*sprite)->scaley = (ONE * 1)*1.15;				// Sprite scale (height)
...
void sprite_create_arrow(unsigned char imageData[], int x, int y, GsSPRITE **sprite) {
...
void initialize_ordering_table(){
	...
	orderingTable[2].length = OT_LENGTH;
	orderingTable[3].length = OT_LENGTH;
	orderingTable[4].length = OT_LENGTH;
	
    orderingTable[0].org = minorOrderingTable[0];
    orderingTable[1].org = minorOrderingTable[1];
	
	orderingTable[2].org = minorOrderingTable[2];
    orderingTable[3].org = minorOrderingTable[3];
	orderingTable[4].org = minorOrderingTable[4];

    GsClearOt(0,0,&orderingTable[0]);
    GsClearOt(0,0,&orderingTable[1]);
	GsClearOt(0,0,&orderingTable[2]);
	GsClearOt(0,0,&orderingTable[3]);
	GsClearOt(0,0,&orderingTable[4]);
}
...
void initialize_debug_font() {
	FntLoad(960, 256);
	SetDumpFnt(FntOpen(5, 20, 320, 240, 0, 512)); //Sets the dumped font for use with FntPrint(); 5, 20, 320, 240, 0, 512
}

void hit_font(hitposx,hitposy) {

	FntLoad(960, 256);
	SetDumpFnt(FntOpen(hitposx, hitposy-10, 320, 240, 0, 512)); //Sets the dumped font for use with FntPrint(); 5, 20, 320, 240, 0, 512
	
	//SetDumpFnt(FntOpen(500, 500, 320, 240, 0, 512));
